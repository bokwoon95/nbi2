  ...
  const initDropdownMenu = (dropdownMenuComponent) => {
    ...
    const closePopover = (focusOnTrigger = true) => {
      ...
      if (supportsManualPopover) {
        ...
        positionFrame = 0;
        scheduledPosition = false;
        if (cleanupPositionListeners.length > 0) {
          cleanupPositionListeners.forEach((removeListener) => removeListener());
    ...
    const openPopover = (initialSelection = false) => {
      ...
      if (supportsManualPopover) {
        ...
        const handlePositionEvent = () => schedulePosition();
        document.addEventListener('scroll', handlePositionEvent, true);
        cleanupPositionListeners.push(() => document.removeEventListener('scroll', handlePositionEvent, true));
        window.addEventListener('resize', handlePositionEvent);
        cleanupPositionListeners.push(() => window.removeEventListener('resize', handlePositionEvent));
        let ancestor = trigger.parentElement;
        while (ancestor && ancestor !== document.body) {
          if (ancestor.scrollHeight > ancestor.clientHeight || ancestor.scrollWidth > ancestor.clientWidth) {
            ancestor.addEventListener('scroll', handlePositionEvent);
            cleanupPositionListeners.push(() => ancestor.removeEventListener('scroll', handlePositionEvent));
          }
